...
const ReviewDetials: React.FC<ReviewsProps> = () => {
  const { profile, token, showToast, isDarkMode } = useAuth();
  const params = useLocalSearchParams();
  const data = params.data ? JSON.parse(params.data as string) : {};
  ...
  const [selectedFilter, setSelectedFilter] = useState<number | null>(null); // null for "All"

  // Build a mapping from rating value to count
  const ratingCounts = reviews.reduce<Record<number, number>>((acc, rev) => {
    const rounded = Math.floor(rev.rating);
  ...
  const userReviews = reviews.filter((r) => r.reviewer_id === profile?.id);
  const otherReviews = reviews.filter((r) => r.reviewer_id !== profile?.id);

  const filteredOtherReviews =
    selectedFilter === null
      ? otherReviews
      : otherReviews.filter((r) => Math.floor(r.rating) === selectedFilter);

  const displayReviews = [...userReviews, ...filteredOtherReviews];
          ...
          {filterOptions.map((option) => (
            ...
            >
              <MaterialIcons
                name="star"
                size={20}
                color={selectedFilter === option.value ? "#fff" : "#FF5ACC"}
              />
              <Text
                style={{
                  color: selectedFilter === option.value ? "#fff" : "#FF5ACC",
              ...
              >
                ...
                {option.value === null && ` (${reviews.length})`}
              </Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
          ...
          >
            <View className="flex-row items-center gap-1">
              ...
              <Text
                ...
                style={{
                  color: isDarkMode ? "#ff5acc" : "#232323",
                }}
              >
                {reviews.length > 0
                  ? (
            ...
            <TouchableOpacity onPress={() => setModalVisible(true)}>
              <Text
                className="font-poppins-semibold text-center text-sm"
                style={{ color: "#ff5acc" }}
              >
                Leave a review
              </Text>
              ...
              <View
                style={[
                  styles.card,
                  {
                    ...
                    borderColor: isDarkMode ? "#403346" : "#eee",
                  },
                ]}
                className="border"
              >
                  ...
                  <View style={styles.row}>
                    ...
                    <View className="flex-row gap-1 items-center justify-center border border-primary rounded-3xl py-1 px-2">
                      <MaterialIcons
                        ...
                        size={18}
                      />
                      <Text className="mt-1" style={styles.rating}>
          ...
          >
            ...
            <TextInput
              style={[
                styles.input,
                {
                  backgroundColor: isDarkMode ? "#18181b" : "#f4f4f4",
